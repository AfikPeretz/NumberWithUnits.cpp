...
namespace ariel {
    ...
    NumberWithUnits::NumberWithUnits(double amount, const string &unit_type) {
        if (unitsType.count(unit_type) > 0) {
            ...
            throw("It is not possible to connect different types of units of measure");
        }
    }

    /*
    This method in the first stage reads from a file and inserts into the variables and in the second stage
    creates the conversion rate between the various units of measurement (such that can be converted)
    */
    ...
    /**
     * @brief This method creates a string that represents the object for the output stream
    ...
    /**
     * @brief This method produces an object from a text file and by reading agreed markings knows
       how to put the information into the different types of variables
     * @param stream 
     * @param num 
     * @return istream& 
     */
